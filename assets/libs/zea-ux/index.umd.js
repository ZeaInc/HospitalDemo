...
`
   ...
   * @extends {Operator}
   */
  class MeasurementOperator extends zeaEngine.Operator {
    /**
    ...
    constructor(name) {
      ...
      this.addInput(new zeaEngine.OperatorInput(IO_NAMES.StartXfo));
      this.addInput(new zeaEngine.OperatorInput(IO_NAMES.EndXfo));
      this.addOutput(new zeaEngine.OperatorOutput(IO_NAMES.Distance));
      this.addOutput(new zeaEngine.OperatorOutput(IO_NAMES.LabelXfo));
      this.addOutput(new zeaEngine.OperatorOutput(IO_NAMES.LineXfo));
    }
    ...
    evaluate() {
      ...
      const newLabelXfo = new zeaEngine.Xfo(midPoint);
      newLabelXfo.ori.setFromDirectionAndUpvector(vector, new zeaEngine.Vec3(vector.z, vector.x, vector.y));
  ...
  zeaEngine.Registry.register('MeasurementOperator', MeasurementOperator);
  ...
  class MeasurementHandle extends ScreenSpaceMovementHandle {
    ...
    onDragStart(event) {
      ...
      this.getOwner().traverse((item) => {
        if (item instanceof zeaEngine.GeomItem) {
    ...
    onDragEnd(event) {
      ...
      this.getOwner().traverse((item) => {
        if (item instanceof zeaEngine.GeomItem) item.getParameter('Material').getValue().visibleInGeomDataBuffer = true;
      });
      event.viewport.renderGeomDataFbo();
    }
  }

  zeaEngine.Registry.register('MeasurementHandle', MeasurementHandle);
   ...
   * @extends {TreeItem}
   */
  class Measurement extends zeaEngine.TreeItem {
    /**
     * Creates an instance of Measurement.
     * @param {string} name
     * @param {Color} color
     */
    constructor(name = 'Measurement', color = new zeaEngine.Color('#FCFC00')) {
      super(name);
      ...
      const billboard = new zeaEngine.BillboardItem('DistanceBillboard', this.label);
      billboard.getParameter('LocalXfo').setValue(new zeaEngine.Xfo());
      ...
      const lineGeomItem = new zeaEngine.GeomItem('Line', line, this.lineMaterial);
  ...
  zeaEngine.Registry.register('Measurement', Measurement);
  ...
  class MeasurementChange extends Change {
    ...
    fromJSON(j, context) {
      ...
      const startXfo = new zeaEngine.Xfo();
   ...
   * @extends {BaseTool}
   */
  class MeasurementTool extends zeaEngine.BaseTool {
    /**
    ...
    constructor(appData) {
      ...
      this.colorParam = this.addParameter(new zeaEngine.ColorParameter('Color', new zeaEngine.Color('#FCFC00')));
      if (!appData) console.error('App data not provided to tool');
      this.appData = appData;
  ...
  exports.MeasurementOperator = MeasurementOperator;
